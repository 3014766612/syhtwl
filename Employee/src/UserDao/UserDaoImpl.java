...
public class UserDaoImpl extends Basedao implements UserDaos {
	...
	public List<product> productseleall() {
		...
		conn = getConnection();
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT id,ptname FROM product");
	...
	@Override
	public int upchanpin(product ys) {
		String sql = "UPDATE `product` SET ptname=? WHERE id=?";
		// 2.提供替换?的object[]
		Object[] obj = { ys.getPtname(), ys.getId() };
		return this.getSIDU(sql, obj);
	}

	@Override
	public product upprod(int id) {
		conn = this.getConnection();
	...
	@Override
	public int inschanpin(product ys) {
		conn = this.getConnection();
		String sql = "INSERT INTO `product`(ptname) VALUES(?)";
		// 2.提供替换?的object[]
		Object[] obj = { ys.getPtname()};
		// 执行新增操作
		return this.getSIDU(sql, obj);
	}
